...
# Job parameters
#SBATCH --job-name=joiretJOBexp
#SBATCH --output=joiretEXP.log
...
#initRATE=(4.3e-6 6e-6) # initiation rate (average reference for all transcripts)
...
for j in {0..7}
	...
	for i in {0..7}
		do
		srun -N1 -n1 -c1 --exact python ${SOFTDIR}/myProg02U34.py ${DATADIR}/dataJSONyeast.json ${DATADIR}/tunnelElectrostaticsAF.json \
		${DATADIR}/CDSfasta01.txt ${FILES[$SLURM_ARRAY_TASK_ID]} ${OUTPUTDIR}/U34ratio${i}init${j}res$SLURM_ARRAY_TASK_ID.txt ${RATIO[$i]} ${initRATE[$j]} &
		done
		wait
	done
